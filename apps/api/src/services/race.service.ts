...
export class CodeService {
  ...
  async generate(maxUses: number, adminId: string) {
  ...
  async validate(code: string, userId: string) {
    ...
    if (!record) throw new ServiceError("Code not found", 404);
  ...
  async delete(code: string, adminId: string) {
    const ok = await this.repos.codes.delete(code.toUpperCase());
    if (!ok) throw new ServiceError("Code not found", 404);
...
export class PredictionService {
  ...
  async submit(input: {
    ...
    if (!input.consentAccepted) throw new ServiceError("Consent required", 400);
    const roundError = validateRounds(input.rounds);
    if (roundError) throw new ServiceError(roundError, 400);

    const code = input.code.toUpperCase();
    const existing = await this.repos.predictions.findByUserId(input.userId);
    if (existing) throw new ServiceError("Already submitted", 409);

    const usage = await this.repos.codes.getUsage(code, input.userId);
    if (!usage?.validatedAt) throw new ServiceError("Validate code first", 403);
    if (usage.submitted) throw new ServiceError("Already submitted for this code", 409);
  ...
  async checkSubmission(userId: string) {
...
export class RaceService {
  ...
  async saveWinners(rounds: number[], adminId: string) {
  ...
  async getLeaderboard(includePredictions = false): Promise<{ entries: LeaderboardEntry[]; etag: string }> {
    const cacheKey = CACHE_KEYS.leaderboard(includePredictions);
    const cached = cache.get<LeaderboardEntry[]>(cacheKey);
    if (cached) return { entries: cached.data, etag: cached.etag };

    const participants = await this.repos.participants.listOrdered();
    let entries: LeaderboardEntry[] = participants.map((p, i) => ({ ...p, rank: i + 1 }));
  ...
  async deleteAllParticipants(adminId: string) {
