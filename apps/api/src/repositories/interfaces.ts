...
export interface UserRepository {
  ...
  create(data: Omit<User, "createdAt" | "updatedAt"> & { passwordHash: string }): Promise<User>;
...
export interface RefreshTokenRepository {
  ...
  findByHash(tokenHash: string): Promise<{ userId: string; expiresAt: string } | null>;
...
export interface ParticipantRepository {
  ...
  bulkUpdateScores(updates: Array<{ userId: string; score: number }>, lastScoredAt: string): Promise<void>;
  deleteAll(): Promise<number>;
}
...
export interface AuditLogRepository {
  ...
  list(limit?: number): Promise<Array<{ id: string; action: string; userId: string | null; details: Record<string, unknown>; createdAt: string }>>;
}
